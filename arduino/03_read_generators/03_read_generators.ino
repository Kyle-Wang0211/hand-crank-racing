// 测试3: 从两个手摇发电机读真实电压 → 通过分压器进 A2/A7
// 协议: 每 50ms 发一行 "v1,v2\n" (两个 0~1000 之间的归一化值)
//
// 硬件回顾:
//   电机1(+) --[22kΩ]--●-- A2     中点 ● 接 A2
...
void loop() {
  ...
  (void)analogRead(A2);         // 空读: 把 ADC 内部切到 A2 并稳定下来
  delayMicroseconds(50);
  int raw1 = analogRead(A2);    // 真读 (玩家1)

  (void)analogRead(A7);         // 空读
  delayMicroseconds(50);
  int raw2 = analogRead(A7);    // 真读 (玩家2)
